...
const FIELD_HELPERS: Record<keyof Omit<SiteSettings, "id">, string> = {
  company_name: "Site alt bölümü, SEO ve site genelindeki marka alanlarında kullanılır.",
  phone: "İletişim sayfası, site alt bölümü ve mobil arama butonunda kullanılır.",
  whatsapp_number: "WhatsApp butonları ve iletişim yönlendirmelerinde kullanılır.",
  email: "İletişim sayfası ve site alt bölümünde görünür.",
  address: "İletişim sayfası, site alt bölümü ve konum bilgilerinde kullanılır.",
  map_embed_url: "İletişim sayfasındaki harita önizlemesinde kullanılır.",
  instagram_url: "Site alt bölümü sosyal medya ikonunda kullanılır.",
  facebook_url: "Site alt bölümü sosyal medya ikonunda kullanılır.",
  linkedin_url: "Site alt bölümü sosyal medya ikonunda kullanılır.",
  ...
  favicon_url: "Tarayıcı sekmesi, Google ve kısayol simgelerinde kullanılır.",
...
export default function AdminSettings() {
  ...
  const validation = useMemo(() => {
    ...
    if (favicon && !/^\/|https?:\/\//i.test(favicon)) {
      warnings.favicon_url = "Site simgesi yolu / veya http:// ya da https:// ile başlamalıdır.";
    }
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="space-y-6">
          <SettingsSection title="Firma Bilgileri" description="Marka adı ve site alt bölümünde kullanılan kurumsal açıklama.">
            ...
            <SettingField label="Site Alt Bölümü Açıklaması" field="footer_description" value={data.footer_description || ""} onChange={(value) => up("footer_description", value)} textarea count />
            <div>
              <div className="mb-1 flex items-center justify-between gap-3">
              ...
              <div className="grid gap-3 sm:grid-cols-[72px_minmax(0,1fr)]">
                <div className="flex h-[72px] w-[72px] items-center justify-center rounded-md border border-border bg-surface-light">
                  <img src={resolveImageUrl(data.favicon_url || "/favicon.png")} alt="Site simgesi önizleme" className="h-10 w-10 object-contain" />
                </div>
          ...
          <SettingsSection title="İletişim ve Satış Kanalları" description="Ziyaretçilerin size ulaşmasını sağlayan telefon, WhatsApp, e-posta ve adres bilgileri.">
            ...
            <PreviewPanel title="WhatsApp Bağlantı Önizlemesi" description="Web sitesindeki WhatsApp butonları bu bağlantı mantığını kullanır.">
          ...
          <SettingsSection title="Ana Sayfa Karşılama Alanı" description="Ana sayfanın ilk ekranda verdiği ana mesajı yönetin.">
            <SettingField label="Karşılama Başlığı" field="hero_title" value={data.hero_title || ""} onChange={(value) => up("hero_title", value)} count />
            <SettingField label="Karşılama Alt Başlığı" field="hero_subtitle" value={data.hero_subtitle || ""} onChange={(value) => up("hero_subtitle", value)} textarea count />
          ...
          <SettingsSection title="Sosyal Medya" description="Site alt bölümündeki sosyal medya bağlantılarını yönetin. Boş bırakılan bağlantılar sitede görünmez.">
            <div className="grid gap-5 md:grid-cols-3">
              <SettingField label="Instagram Bağlantısı" field="instagram_url" value={data.instagram_url || ""} onChange={(value) => up("instagram_url", value)} warning={validation.warnings.instagram_url} />
              <SettingField label="Facebook Bağlantısı" field="facebook_url" value={data.facebook_url || ""} onChange={(value) => up("facebook_url", value)} warning={validation.warnings.facebook_url} />
              <SettingField label="LinkedIn Bağlantısı" field="linkedin_url" value={data.linkedin_url || ""} onChange={(value) => up("linkedin_url", value)} warning={validation.warnings.linkedin_url} />
            </div>
          </SettingsSection>
            ...
            <PreviewPanel title="Google Site Bağlantıları Hedef Önizlemesi" description="Ana marka aramasında hedeflenen bağlantı mimarisinin temsili görünümü.">
              <div className="rounded-lg border border-border bg-white p-4">
                ...
                <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
                  Google site bağlantılarını garanti etmez. Bu alan, doğru site mimarisi ve SEO çalışmasıyla hedeflenen görünümü temsil eder.
                </p>
              </div>
            </PreviewPanel>
          </SettingsSection>

          <SettingsSection title="Yönetici Bildirimleri" description="Bu cihaza özel yönetim paneli tarayıcı bildirimleri.">
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-lg border border-border bg-card p-5 shadow-card-soft">
            ...
            <div className="mt-5 space-y-3">
              {[
                ["Ana sayfa karşılama başlığı", "Aktif", "success"],
                ["Ana sayfa karşılama açıklaması", "Aktif", "success"],
                ...
                ["Sohbet robotu WhatsApp entegrasyonu", "Ayrı yapı / kontrol gerekli", "warning"],
          ...
          <section className="rounded-lg border border-border bg-card p-5 shadow-card-soft">
            ...
            <div className="mt-4">
              <SettingField label="Google Haritalar Yerleştirme Bağlantısı" field="map_embed_url" value={data.map_embed_url || ""} onChange={(value) => up("map_embed_url", value)} warning={validation.warnings.map_embed_url} />
              {canPreviewMap ? (
                ...
                <div className="mt-4">
                  <AdminEmptyState
                    ...
                    description={mapUrl ? "Önizleme için bağlantı http:// veya https:// ile başlamalıdır." : "Google Haritalar yerleştirme bağlantısı eklendiğinde iletişim sayfasında harita görünür."}
                    icon={MapPin}
                  />
                </div>
              )}
